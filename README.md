...
[![Release](https://img.shields.io/badge/Release-0.3.4-blue.svg)](./CHANGELOG.md)
...
```bash
./install.sh --version v0.3.4     # pin an exact tag
./install.sh --channel main       # track edge
./install.sh --force              # discard local edits + stale build artifacts, then rebuild
```

> **Where `main` stands:** the default channel installs the newest release tag (**0.3.4**).
> Workspace-role enforcement, API-token scopes and the manifest's build-shaping sections landed after
> that tag and ship in the next release — see [`CHANGELOG.md`](./CHANGELOG.md) under *Unreleased*, or
> run `--channel main` to get them today.
