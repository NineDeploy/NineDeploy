...
/**
 * Periodic housekeeping: prunes deploy-log files and time-series/audit tables so
 * a long-running instance doesn't slowly fill its disk. Metrics retention is
 * handled by the collector plugin; this covers logs + audit + notification log.
 */
export default fp(
  async (fastify) => {
    let running = true;
